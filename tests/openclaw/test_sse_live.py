...
# -*- coding: utf-8 -*-
"""
Live SSE streaming E2E for the docker-a2a-gateway stack (OpenClaw bridge).
...
Verifies the full streaming pipeline end-to-end:
  ...
  07  Direct OpenClaw streaming chat/completions verification

Each step prints PASS/FAIL. Exits non-zero if any step failed.

Usage:
    python tests/openclaw/test_sse_live.py
    python tests/openclaw/test_sse_live.py --prompt "Stream a haiku about the sea."
    python tests/openclaw/test_sse_live.py --timeout 200
...
def main():
    parser = argparse.ArgumentParser(
        description="Live SSE streaming E2E for the A2A OpenClaw gateway stack"
    )
    ...
    print(f"{B}{'=' * 70}{RST}")
    print(f"{C}OpenClaw SSE Live Streaming E2E{RST}")
    print(f"{B}{'=' * 70}{RST}")
    ...
    if not args.no_health:
        ...
        _record("openclaw /v1/models", o_ok,
                f"({cfg['openclaw_url']})" if o_ok else f"({cfg['openclaw_url']})")
    ...
    params = message_send_params(
        ...
        task_type="openclaw_sse_live", stream=True,
    )
    r = send_a2a(
    ...
    # 07: Direct OpenClaw streaming chat/completions verification
    _section("07 — Direct OpenClaw streaming chat/completions")
    try:
        openclaw_r = requests.post(
            f"{cfg['openclaw_url']}/v1/chat/completions",
            headers={
                "Authorization": f"Bearer {cfg['openclaw_key']}" if cfg["openclaw_key"] else "",
        ...
        if openclaw_r.status_code == 200:
            openclaw_text = ""
            for line in openclaw_r.iter_lines(decode_unicode=True):
                if not line:
                    continue
                if line.startswith("data: "):
                    data_str = line[6:]
                    ...
                    try:
                        ...
                        if choices:
                            ...
                            if delta_content:
                                openclaw_text += delta_content
                    except json.JSONDecodeError:
                        pass
            _record("direct openclaw streaming", bool(openclaw_text),
                    f"chars={len(openclaw_text)}" if openclaw_text else "no text")
        else:
            _record("direct openclaw streaming", False,
                    f"HTTP {openclaw_r.status_code}")
    except Exception as e:
        _record("direct openclaw streaming", False, str(e))

...
# -*- coding: utf-8 -*-
"""
One-shot "say hello from OpenClaw" through A2A, streamed back over SSE.

    Client -> POST /{endpoint_id}/a2a (message/send, stream=true)
    -> SilvaEngine Gateway -> A2ADaemonExecutor -> OpenClawAgentHandler
    -> OpenClaw Gateway (POST /v1/chat/completions with stream:true SSE)
...
Usage:
    python tests/openclaw/test_hello_sse.py
    python tests/openclaw/test_hello_sse.py --prompt "Greet me in one sentence."
    python tests/openclaw/test_hello_sse.py --timeout 200
...
DEFAULT_PROMPT = "Say hello from OpenClaw through A2A. Reply in one short sentence."
...
def main():
    parser = argparse.ArgumentParser(
        description="One-shot streaming E2E: say hello from OpenClaw via A2A + SSE"
    )
    ...
    print(f"{B}{'=' * 70}{RST}")
    print(f"{C}OpenClaw Hello (SSE streaming){RST}")
    print(f"{B}{'=' * 70}{RST}")
    ...
    params = message_send_params(
        ...
        task_type="openclaw_hello_sse", stream=True,
    )
    r = send_a2a(
